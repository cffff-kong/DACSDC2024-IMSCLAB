...
class YoloDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        mask_8 = np.where(mask == 8, 1, 0)  
        mask_9 = np.where(mask == 9, 1, 0)   
        mask_10 = np.where(mask == 10, 1, 0) 
        mask_new = np.stack([mask_8, mask_9, mask_10], axis=-1)
        # mask_F = np.expand_dims(mask, axis=-1) - 1
        # mask_F[mask_F == -1 ] =255
        mask_F = np.expand_dims(mask, axis=-1) 
        #mask_F[mask_F == -1 ] =255
        ...
        return image, mask_new, mask_F, labels_out
...
def yolo_dataset_collate(batch):
    ...
    masks  = []
    masks_F = []
    for i, (img, mask, mask_F, box) in enumerate(batch):
        images.append(img)
        masks.append(mask)
    ...
    masks_F = torch.from_numpy(np.array(masks_F)).type(torch.FloatTensor)
    # masks_F = np.expand_dims(np.array(masks_F, dtype=np.float32, copy=False), axis=2) - 1
    # print(mask_F.shape)
    # masks_F = torch.from_numpy(masks_F.transpose((2, 0, 1))).float()
    bboxes  = torch.from_numpy(np.concatenate(bboxes, 0)).type(torch.FloatTensor)
    return images, masks, masks_F, bboxes
...
#     def __init__(self, seg_lines, input_shape, epoch_length):
#         super(SegDataset, self).__init__()
#         self.seg_lines = seg_lines  
#         self.input_shape = input_shape  
#         self.epoch_length = epoch_length

#         self.length = len(self.seg_lines)
