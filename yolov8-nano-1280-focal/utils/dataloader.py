...
class YoloDataset(Dataset):
    def __init__(self, annotation_lines, segmentation_lines, input_shape, num_classes, epoch_length, \
                        mosaic, mixup, mosaic_prob, mixup_prob, train, special_aug_ratio = 0.7):
        super(YoloDataset, self).__init__()
        self.annotation_lines   = annotation_lines
        #segmentation_lines is a list of paths to segmentation masks,编写代码读取segmentation_lines的第二列作为self.seg_lines
        
        self.seg_lines          = segmentation_lines
    ...
    def __getitem__(self, index):
        ...
        if self.mosaic and self.rand() < self.mosaic_prob and self.epoch_now < self.epoch_length * self.special_aug_ratio:
            ...
            shuffle(lines)
            seg_lines = sample(self.seg_lines, 3)
            seg_lines.append(self.seg_lines[index])
            shuffle(seg_lines)
            image, mask, box = self.get_random_data_with_Mosaic(lines, seg_lines, self.input_shape)
    ...
    def get_random_data(self, annotation_line, seg_line, input_shape, jitter=.3, hue=.1, sat=0.7, val=0.4, random=True):
        ...
        img_path, mask_path = seg_line.split()
        #------------------------------#
        #   读取图像并转换成RGB图像
        #------------------------------#
    ...
    def get_random_data_with_Mosaic(self, annotation_line, segmentation_line, input_shape, jitter=0.3, hue=.1, sat=0.7, val=0.4):
        h, w = input_shape
        ...
        box_datas   = []
        index       = 0
        for line, seg_lines in zip(annotation_line, segmentation_line):
            #---------------------------------#
            #   每一行进行分割
            #---------------------------------#
            line_content = line.split()
            imgs_content, seg_content  = seg_lines.split()
            #---------------------------------#
            #   打开图片
            #---------------------------------#
            image = Image.open(line_content[0])
            image = cvtColor(image)
            mask  = Image.open(seg_content).convert("L")
