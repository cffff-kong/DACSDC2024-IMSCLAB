...
from utils.dataloader import YoloDataset, yolo_dataset_collate, SegDataset, seg_dataset_collate
...
##################################################################################2024.4.8
    ...
    # 假设 ema 是 ModelEMA 的实例
    # for name, param in ema.ema.state_dict().items():
    ...
    seg_num_train   = len(seg_train_lines)
    seg_num_val     = len(seg_val_lines)
    ...
    if True:
        ...
        ############################################################################################输出应该是8100
        seg_train_dataset = SegDataset(seg_train_lines, input_shape, epoch_length=UnFreeze_Epoch)
        seg_val_dataset = SegDataset(seg_val_lines, input_shape, epoch_length=UnFreeze_Epoch)
        # print(f'seg_train_dataset: {len(seg_train_dataset)}')
        # print(f'seg_train_lines: {len(seg_train_lines)}')
        # print(f'seg_val_dataset: {len(seg_val_dataset)}')
        # print(f'seg_val_lines: {len(seg_val_lines)}')

        # print(f'seg_train_dataset: {seg_train_dataset[0]}')
        ...
        labels =        DataLoader(seg_train_dataset, batch_size = batch_size, shuffle=shuffle, num_workers=num_workers, 
                                 collate_fn=seg_dataset_collate)
        labels_val =    DataLoader(seg_val_dataset, batch_size = batch_size, shuffle=shuffle, num_workers=num_workers,
                                        collate_fn=seg_dataset_collate)
        #----------------------#
        #   记录eval的map曲线
        #----------------------#
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            fit_one_epoch(model_train, model, ema, yolo_loss, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, labels, labels_val, UnFreeze_Epoch, Cuda, fp16, scaler, save_period, save_dir, local_rank)
