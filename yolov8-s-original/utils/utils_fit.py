...
def fit_one_epoch(model_train, model, ema, yolo_loss, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch, cuda, fp16, scaler, save_period, save_dir, local_rank=0):
    ...
    val_det_loss = 0
    val_seg_loss = 0
    # para_seg = math.ceil(200/(epoch+1))
    para_seg = 100
    predlist = []
    gts = []
    ...
    # print(f"Total number of parameters: {total_params/1e6:,}")
    ...
    for iteration, batch in enumerate(gen):
        ...
        with torch.no_grad():
            if cuda:
                ...
                # print(f'images shape: {images.shape}, masks shape: {masks.shape}, bboxes shape: {bboxes.shape}')
        #----------------------#
        #   清零梯度
        #----------------------#
        optimizer.zero_grad()
        if not fp16:
            ...
            fea_img = torch.argmax(outputs[1].long(), 1)
            fea_img = fea_img[0, :, :].cpu().detach().numpy()
            # print(fea_img)
            fea_img = np.array(fea_img)
            plt.imshow(fea_img, cmap='gray')
            plt.savefig('./logs/output_fea.png')
            plt.close()           
            mask_img = masks[0, 0, :, :].cpu().detach().numpy()
            # print(fea_img)
            mask_img = np.array(mask_img)
            plt.imshow(mask_img, cmap='gray')
            plt.savefig('./logs/mask.png')
            plt.close()
            #################################################################画mask对比

            det_value = yolo_loss(outputs[0], bboxes)
            seg_value = para_seg*seg_loss(outputs[1], masks.long().squeeze(1))
            loss_value = det_value + seg_value
            #----------------------#
            #   反向传播
            #----------------------#
            loss_value.backward()
        ...
        if local_rank == 0:
            pbar.set_postfix(**{'loss'  : loss_value.item(), 
                                'det_loss'  : det_value.item(),
                                'seg_loss*100'  : seg_value.item(),
                                'lr'    : get_lr(optimizer)})
            pbar.update(1)
    ...
    for iteration, batch in enumerate(gen_val):
        ...
        val_det_loss += det_value.item()
        val_seg_loss += seg_value.item()
        ...
        if local_rank == 0:
            pbar.set_postfix(**{'val_loss': val_loss / (iteration + 1),
                                'val_det_loss': val_det_loss / (iteration + 1),
                                'val_seg_loss': val_seg_loss / (iteration + 1)})
            pbar.update(1)
            
    score = scores(gts, predlist, 4)
            
    if local_rank == 0:
        pbar.close()
        print('Finish Validation')
        loss_history.append_loss(epoch + 1, loss / epoch_step, val_loss / epoch_step_val, val_det_loss / epoch_step_val, score, val_seg_loss / epoch_step_val)
        eval_callback.on_epoch_end(epoch + 1, model_train_eval)
        print('Epoch:'+ str(epoch + 1) + '/' + str(Epoch))
        print('Total Loss: %.3f || Val Loss: %.3f || Val det loss: %.3f || Val seg loss: %.3f' % (loss / epoch_step, val_loss / epoch_step_val, val_det_loss / epoch_step_val, val_seg_loss / epoch_step_val))
