...
class YOLO(object):
    ...
    def detect_image(self, image, crop = False, count = False):
        ...
        print(f"Total number of parameters (in millions): {num_params_m:.2f}M")
        num_params2 = sum(p.numel() for name, p in self.net.named_parameters() if 'backbone' in name)
        num_params_m2 = num_params2 / 1e6
        print(f"Total number of backbone parameters (in millions): {num_params_m2:.2f}M")
        ##############################################################输出参数量
        with torch.no_grad():
            ...
            new_height = int(self.input_shape[0] / target_aspect)
            offset = (self.input_shape[1] - new_height) // 2
            masks = torch.argmax(outputs[1].long(), 1)
            ...
            #     raise ValueError("The 'masks' input is empty or None.")
            # if masks.dtype != np.uint8:
            #     masks = masks.astype(np.uint8)  
            # if len(masks.shape) != 2:
            #     raise ValueError("The 'masks' input should have two dimensions for grayscale images.")
            # mask = mask.astype(np.uint8)
            # color_map = {1: (255, 0, 0), 2: (0, 255, 0), 3: (0, 0, 255)}
            # masks = masks.astype('uint8')
            # masks = Image.fromarray(masks).convert('RGB')
            #######################################################################我加的
