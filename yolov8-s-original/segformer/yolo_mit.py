...
from nets.backbone_mit import yolo_mit
...
class YoloBody(nn.Module):
    def __init__(self, input_shape, num_classes, phi, pretrained=False):
        ...
        self.last_layer_depth = nn.Sequential(
            nn.Conv2d(out_channels, out_channels, kernel_size=3, stride=1, padding=1),
            nn.ReLU(inplace=False),
            nn.Conv2d(out_channels, 4, kernel_size=3, stride=1, padding=1))
        ########################
    ...
    def forward(self, x):
        #  backbone
        feat1, feat2, feat3, feat4 = self.backbone.forward(x)
        # print(feat1.shape, feat2.shape, feat3.shape)

        # y = self.linear_pred(feat1)
        # y = self.up(y)
        # y = self.up(y)
        # seg_result = self.up(y)
        ######################################################################################改
        n, _, h, w = feat3.shape
        # c3 = feat3
        # c2 = feat2
        ...
        # # print(_c3.shape, _c2.shape, _c1.shape)
        ...
        x_1_ = self.skip_conv2(x_1)
        out = self.fusion2(x_1_, out)
        out = self.up(out)

        # x_1_ = self.skip_conv3(x_1)
        # out = self.fusion3(x_1_, out)
        # out = self.up(out)
        out = self.up(out)
        seg_result = self.up(out)

        seg_result = self.last_layer_depth(seg_result)
        #x = self.dequant(x)
        ######################################################################################改
        ...
        result1 = dbox, cls, x, self.anchors.to(dbox.device), self.strides.to(dbox.device)
        result = (result1), seg_result
        return result
    ######################################################################################改
class MLP(nn.Module):
    """
